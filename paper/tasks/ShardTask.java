...
public class ShardTask implements Runnable {
    ...
    public void run() {
        ...
        plugin.getPaperScheduler().forEachOnlinePlayer((Player player) -> tickPlayer(player, sm));
    }
    ...
    public static void start(UltimateDonutSmp plugin) {
        plugin.getPaperScheduler().runGlobalTimer(new ShardTask(plugin), 60 * 20L, 60 * 20L); // every minute
    }
}
